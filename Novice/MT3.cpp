	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
			...
			Vector3 a{std::cos(lat) * std::cos(lon) * sphere.radius + sphere.center.x, std::sin(lat) * sphere.radius + sphere.center.y, std::cos(lat) * std::sin(lon) * sphere.radius + sphere.center.z}, 
			    ...
			    c{std::cos(lat) * std::cos(lon + kLonEvery) * sphere.radius + sphere.center.x, std::sin(lat) * sphere.radius + sphere.center.y, std::cos(lat) * std::sin(lon + kLonEvery) * sphere.radius + sphere.center.z};
			//拡縮の処理
			Vector3 a1, b1, c1{};
